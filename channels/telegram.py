...
from picobot.config.paths import get_media_dir
from picobot.config.schema import TelegramConfig
from picobot.utils.helpers import split_message
...
def _normalize_telegram_text(text: str) -> str:
    """Light cleanup for Telegram readability without changing meaning."""
    if not text:
        return ""
    text = text.replace("\r\n", "\n").strip()
    text = re.sub(r"\n{3,}", "\n\n", text)
...
class TelegramChannel(BaseChannel):
    ...
    def is_allowed(self, sender_id: str) -> bool:
        """Check if sender is allowed. Supports both plain ID and id|username format."""
    ...
    async def start(self) -> None:
        ...
        self._app.add_handler(CommandHandler("last_error", self._forward_command))
    ...
    async def send(self, msg: OutboundMessage) -> None:
        ...
        if msg.content and msg.content != "[empty message]":
            ...
            inline_buttons = msg.inline_buttons if hasattr(msg, 'inline_buttons') else None
            for chunk in split_message(msg.content, TELEGRAM_MAX_MESSAGE_LEN):
                # Final response: simulate streaming via draft, then persist
                if not is_progress:
                    await self._send_with_streaming(
                        chat_id, chunk, msg.metadata, reply_params, thread_kwargs, inline_buttons
                    )
                else:
    ...
    async def _send_text(
        ...
        """Send a plain text message with HTML fallback and optional inline buttons."""
    ...
    async def _send_with_streaming(
        ...
        try:
            ...
            pass
        await self._send_text(chat_id, text, metadata, reply_params, thread_kwargs, inline_buttons)
    ...
    async def _on_start(self, update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
        ...
        await update.message.reply_text(
            f"👋 Hi {user.first_name}! I'm picobot.\n\n"
    ...
    def _apply_utility_footer(self, text: str, metadata: dict | None) -> str:
        """Append optional Telegram utility footer to final responses."""
        if not text or not self.config.show_model_footer or not metadata:
            return text
        if metadata.get("_progress"):
            return text
        served_by = metadata.get("served_by")
        served_model = metadata.get("served_model")
        if served_by != "openai_codex":
            return text
        label = "codex fallback"
        if served_model and "codex" not in str(served_model).lower():
            label = str(served_model)
        return f"{text}\n\n({label})"
