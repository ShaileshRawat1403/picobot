...
app = typer.Typer(
    name="picobot",
    help=f"{__logo__} picobot - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from picobot.config.paths import get_cli_history_path
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} picobot[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(response: str, render_markdown: bool) -> None:
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} picobot[/cyan]"),
...
def _load_runtime_config(config: str | None = None, workspace: str | None = None) -> Config:
    ...
    from picobot.config.loader import load_config, set_config_path
...
def _print_deprecated_memory_window_notice(config: Config) -> None:
    ...
    if config.agents.defaults.should_warn_deprecated_memory_window:
        console.print(
            ...
            "[cyan]picobot onboard[/cyan] to refresh your config template."
        )
...
def _run_gateway(config: "Config", verbose: bool = False) -> None:
    ...
    console.print(f"{__logo__} Starting picobot gateway on port {port}...")
...
def agent(
    ...
    if logs:
        logger.enable("picobot")
    else:
        logger.disable("picobot")
...
def channels_status():
    """Show channel status."""
    from picobot.channels.registry import discover_channel_names, load_channel_class
...
def _get_bridge_dir() -> Path:
    ...
    from picobot.config.paths import get_bridge_install_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # picobot/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall picobot")
        raise typer.Exit(1)
...
def channels_login():
    ...
    from picobot.config.loader import load_config
    from picobot.config.paths import get_runtime_subdir
...
def doctor():
    ...
    if version.major >= 3 and version.minor >= 8:
        ...
        console.print(f"[red]✗[/red] Python: {version.major}.{version.minor} (requires 3.8+)")
    ...
    try:
        ...
        if workspace.exists():
            ...
            console.print(f"[yellow]![/yellow] Workspace: {workspace} (will be created)")
            issues.append("Workspace not initialized")
    except Exception as e:
        console.print(f"[red]✗[/red] Workspace: Error loading config - {e}")
    ...
    for spec in PROVIDERS:
        ...
        if spec.is_oauth:
            ...
            if p.api_base:
                console.print(f"  {spec.label}: [green]✓[/green] {p.api_base}")
                checks_passed += 1
                has_any_key = True
            else:
                console.print(f"  {spec.label}: [dim]not configured[/dim]")
        elif p.api_key:
            masked = p.api_key[:8] + "..." + p.api_key[-4:] if len(p.api_key) > 12 else "***"
            console.print(f"  {spec.label}: [green]✓[/green] {masked}")
            checks_passed += 1
            has_any_key = True
        else:
            console.print(f"  {spec.label}: [dim]not set[/dim]")
    ...
    if not has_any_key:
        ...
        console.print("\n[yellow]Warning: No API keys found![/yellow]")
        console.print("  Run `picobot onboard` or set keys in config.json")
    ...
    if shutil.which("tmux"):
        ...
        console.print("\n[yellow]![/yellow] tmux: not found (optional)")
        issues.append("tmux not installed")

    # Summary
    console.print(f"\n[bold]Summary:[/bold] {checks_passed} checks passed, {len(issues)} issues")
    if issues:
        console.print("\n[bold yellow]Issues to fix:[/bold yellow]")
        for issue in issues:
