...
if TYPE_CHECKING:
    from picobot.config.schema import WebSearchConfig
...
class WebSearchTool(Tool):
    ...
    async def execute(self, query: str, count: int | None = None, **kwargs: Any) -> str:
        ...
        async def try_provider(name: str, func) -> str | None:
            ...
            try:
                ...
                if not result.startswith("Error:") and not result.startswith("No results"):
                    return result
                logger.debug("{} failed: {}", name, result[:100])
            except Exception as e:
        ...
        if provider == "duckduckgo":
            result = await try_provider("ddgs", lambda: self._search_duckduckgo(query, n))
            if result:
                return result
            result = await try_provider("ddgs_html", lambda: self._search_duckduckgo_html(query, n))
            if result:
                return result
        elif provider == "tavily":
            result = await try_provider("tavily", lambda: self._search_tavily(query, n))
            if result:
                return result
            result = await try_provider("ddgs", lambda: self._search_duckduckgo(query, n))
            if result:
            ...
            result = await try_provider("searxng", lambda: self._search_searxng(query, n))
            if result:
                return result
        elif provider == "jina":
            result = await try_provider("jina", lambda: self._search_jina(query, n))
            if result:
                return result
        elif provider == "brave":
            result = await try_provider("brave", lambda: self._search_brave(query, n))
            if result:
                return result
            result = await try_provider("ddgs", lambda: self._search_duckduckgo(query, n))
            if result:
        ...
        # Ultimate fallback: direct HTML scraping
        result = await try_provider("ddgs_html_fallback", lambda: self._search_duckduckgo_html(query, n))
        if result:
            return result

        return f"Error: All search providers failed for: {query}"
    ...
    async def _search_duckduckgo(self, query: str, n: int) -> str:
        try:
            ...
            logger.info("ddgs not installed, using direct HTML scraping")
            return await self._search_duckduckgo_html(query, n)
        except Exception as e:
            logger.warning("DuckDuckGo ddgs failed: {}, trying HTML scraping", e)
            return await self._search_duckduckgo_html(query, n)
